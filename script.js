...
el.svgContainer.style.opacity = 0;
...
// Crear el timeline pero NO reproducirlo todavía
const loveTl = crtLoveTl();

/* INTRO */
const introText = qs(".intro-text");
const finalText = qs(".final-text");

const introTl = new mojs.Timeline().add([
  new mojs.Html({
    el: introText,
    duration: 1200,
    opacity: { 0: 1 },
  ...
  new mojs.Html({
    ...
    easing: "quad.in",
    onComplete: () => {
      // Ocultar completamente el intro
      introText.style.display = "none";
      introText.style.visibility = "hidden";
      introText.style.zIndex = "-1";
      
      // Mostrar y reproducir la animación principal
      el.container.style.opacity = 1;
      el.svgContainer.style.opacity = 1;
      loveTl.play();
      
      // Después de la animación principal, mostrar "TE AMO JEAN"
      setTimeout(() => {
        showFinalMessage();
      }, 4100); // Justo antes de que termine el loop
    }
  })
]);

// Función para mostrar "TE AMO JEAN"
const showFinalMessage = () => {
  const finalTl = new mojs.Timeline().add([
    new mojs.Html({
      el: finalText,
      duration: 800,
      opacity: { 0: 1 },
...
}, 7500); // Aumentado para dar tiempo al mensaje final
